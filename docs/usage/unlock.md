# <code>mutation-seek <b>unlock</b></code>
...
This part of the documentation describes options and concepts for <code>mutation-seek <b>unlock</b></code> sub command in more detail. With minimal configuration, the **`unlock`** sub command enables you to unlock a pipeline output directory. 
...
Unlocking mutation-seek pipeline output directory is fast and easy! In its most basic form, <code>mutation-seek <b>unlock</b></code> only has *one required input*.

## 2. Synopsis
```text
$ ./mutation-seek unlock [-h] --output OUTPUT
```
...
mutation-seek unlock --output /data/$USER/output
```